 ...
 */
class DataRowReader extends AbstractDriverProxy
{
    /**
    ...
    private function getInputValue(TableFieldDto $field, array $values): mixed
    {
        ...
        if ($field->type === "enum" || count($enumValues) > 0) {
            ...
            return $this->statement()->getUnconvertedFieldValue($field, $value);
        }
        ...
        if ($this->utils()->isBlob($field) && $this->utils()->iniBool('file_uploads')) {
            $file = $this->pageUi()->getFileContents("fields-$fieldId");
            //! report errors
            return is_string($file) ? $this->engine()->quoteBinary($file) : false;
        }

        return $this->statement()->getUnconvertedFieldValue($field, $value, $function);
    }
