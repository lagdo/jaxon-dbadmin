...
class Util implements UtilInterface
{
     ...
     * @return string
     */
    public function messageQuery(string $query, string $time, bool $failed = false)
    {
        if (\strlen($query) > 1e6) {
     ...
     * @return bool
     */
    public function queryAndRedirect(string $query, string $location = null, string $message = null,
        bool $redirect = false, bool $execute = true, bool $failed = false, string $time = "")
    {
        if ($execute) {
     ...
     * @return bool
     */
    private function queriesAndRedirect(string $location, string $message, bool $redirect)
    {
     ...
     * @param string $location
     * @param string $messageDrop
     * @param string $messageAlter
     * @param string $messageCreate
     * @param string $oldName
     * @param string $newName
     *
     * @return void
     */
    public function dropAndCreate(string $drop, string $create, string $dropCreated,
        string $test, string $dropTest, string $location, string $messageDrop,
        string $messageAlter, string $messageCreate, string $oldName, string $newName)
    {
        if ($oldName == "") {
            $this->queryAndRedirect($drop, $location, $messageDrop);
        } elseif ($oldName == "") {
            $this->queryAndRedirect($create, $location, $messageCreate);
        } elseif ($oldName != $newName) {
            $created = $this->driver->queries($create);
            $this->queriesAndRedirect($location, $messageAlter, $created && $this->driver->queries($drop));
            if ($created) {
                $this->driver->queries($dropCreated);
            }
        } else {
            $this->queriesAndRedirect($location, $messageAlter,
                $this->driver->queries($test) && $this->driver->queries($dropTest) &&
     ...
     * @return void
     */
    public function drop(string $drop, string $location, string $messageDrop)
    {
        $this->queryAndRedirect($drop, $location, $messageDrop);
    }
}
